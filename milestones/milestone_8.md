# Milestone 8: Rare/Common Efficiency Solver (V19) - STABLE

## Status: Fully Functional
The solver has been perfected to handle mixed rarity and mixed level SBCs (e.g., Gold, Silver, Bronze requirements). it implements a "Tier-Smart" rarity logic that maximizes the clearing of low-tier duplicates while protecting high-value assets.

## Key Features (V19)
1.  **Multi-Level Bucket Engine:** Automatically identifies and separates requirements for Gold, Silver, and Bronze players into independent buckets.
2.  **Tier-Smart Rarity Logic:**
    - **Bronze/Silver:** Treats Rares and Commons as a single pool to dump the absolute lowest ratings from Storage first.
    - **Gold:** Strictly conserves Rares, using only the minimum mandatory count and filling the rest with Commons.
3.  **Hard Rating Boundaries:** Enforces strict ceilings (Bronze: 64, Silver: 74, Gold: 82) to prevent higher-tier players from being used in lower-tier buckets.
4.  **Negative Value Priority:** SBC Storage duplicates are always the first choice across all tiers and rarities.
5.  **Ironclad Untradeable Lock:** Strictly filters for `tradable: false`. Tradeable liquid assets are 100% protected.
6.  **Persona Deduplication:** Hardened tracking using `usedPersonaIds` to ensure no player clones ever appear.
7.  **Robust Sync:** Uses the server-side `saveChallenge` flow to force the "Exchange" button to light up permanently.

## Technical Architecture
- **Multi-Pass Fetch:** Performs targeted inventory searches for every required level (Gold/Silver/Bronze) to ensure full inventory discovery.
- **Rule Key Mapping:** Intercepts account-specific rule keys (e.g., Key 17 for Level, Key 25 for Rare) to ensure 100% requirement compliance.
- **Surgical Logging:** Provides real-time decision logs explaining the Tier, Rarity, and Source for every slot filled.
