(async function milestone7DeCloggerV20() {
    const win = (typeof unsafeWindow !== 'undefined' ? unsafeWindow : window);
    const repo = win.repositories.Item;

    console.log("%c--- 🛠️ MILESTONE 7: 84-RATED DE-CLOGGER (V20) ---", "font-size: 16px; font-weight: bold; color: #10b981;");
    console.log("[DIAGNOSTIC] V20: Requirement-Aware Anchor Selection.");
    ...
    const reqs = (challenge.eligibilityRequirements || []).map(r => {
        const col = r.kvPairs._collection || r.kvPairs;
        const rules = [];
        for (let k in col) rules.push({ key: parseInt(k), value: col[k]?.value ?? col[k] });
        return { rules, count: r.count };
    });
    const isTotwRequired = reqs.some(r => r.rules.some(rule => rule.key === 18 && (rule.value === 3 || (Array.isArray(rule.value) && rule.value.includes(3)))));
    console.log(`[DIAGNOSTIC] TOTW Required: ${isTotwRequired}`);
    ...
    const identifyAnchor = async () => {
        if (isTotwRequired) {
            console.log("[DECISION] SBC requires TOTW. Finding lowest untradeable TOTW...");
            ...
            if (totws.length === 0) {
                ...
                deepItems.forEach(p => {
                    if (p && p.id && p.tradable === false && !p.evolutionInfo) {
                        p._sourceType = 'club'; p._personaId = Number(p.definitionId) % 16777216;
                        totws.push(p); if (!fullPool.some(fp => fp.id === p.id)) fullPool.push(p);
                    }
                });
            }
            return totws.sort((a,b) => a.rating - b.rating)[0];
        } else {
            console.log("[DECISION] TOTW NOT required. Using 87/88 Gold Rare as Workhorse Anchor...");
            const workhorses = fullPool.filter(p => p.rating >= 87 && p.rating <= 88 && p.rareflag === 1 && p.tradable === false);
            return workhorses.sort((a,b) => a.rating - b.rating)[0];
        }
    };
    ...
    const getPlayers = (rating, count, sourcePref = null) => {
        const matches = fullPool.filter(p => {
            ...
            // Only use standard Gold (Common/Rare) for clogs
    ...
    if (!anchor) return console.error("❌ No valid Anchor (TOTW or 87/88 Gold) found. Confirm inventory.");
    
    console.log(`[DECISION] Selected Anchor: ${anchor._staticData?.name} (${anchor.rating}) [Source: ${anchor._sourceType}]`);
    usedPersonaIds.add(anchor._personaId); usedIds.add(anchor.id);
    ...
    pattern.forEach(pReq => {
        ...
        fromStorage.forEach(p => { 
            console.log(`[DECISION] Slot ${selectedEntries.length}: ${p._staticData?.name} (${p.rating}) [Storage Clog]`);
        ...
        if (needed > 0) {
            ...
            fromClub.forEach(p => { 
                console.log(`[DECISION] Slot ${selectedEntries.length}: ${p._staticData?.name} (${p.rating}) [Club Fulfill]`);
    ...
    if (selectedEntries.length < 11) {
        ...
        while (selectedEntries.length < 11) {
            ...
            if (!p) break;
            console.log(`[DECISION] Slot ${selectedEntries.length}: ${p._staticData?.name} (${p.rating}) [Fail-Safe]`);
    ...
    selectedEntries.forEach((entry, i) => { if (activeSlots[i]) finalArray[activeSlots[i].index] = entry.item; });
    ...
    win.services.SBC.saveChallenge(challenge).observe({ name: 'SaveSquad' }, (obs, res) => {
        if (res.success) {
            console.log("%c[DE-CLOGGER] V20: SAVED SUCCESSFULLY.", "font-weight: bold; color: #10b981;");
